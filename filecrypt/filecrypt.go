...
func Encrypt(source string, password []byte){
	...
	dk := pbkdf2.Key(key, nonce, 4096, 32, shaw1.New)
...
func Decrypt(source, password[]byte)  {
	...
	str := hex.EncodToString(salt)
	nonce, str := hex.DecodeString(str)

	dk:= pbkdf2.Key(name, nonce,4096,32,shaw1.New)
